module mod_usr  ! computing nonlinear force-free field 
  use mod_mhd
  implicit none
...
contains
  ...
  subroutine initglobaldata_usr()
    ...
    call init_b_nlfff_data('nlfff_boundary/nlfff_boundary.dat',unit_length,unit_magneticfield)
  end subroutine initglobaldata_usr

  !==============================================================================
  ! Purpose: to read vector magnetic field from a input file
  !==============================================================================
  subroutine init_b_nlfff_data(magnetogramname,qLunit,qBunit)
    ...
    if(mype==0) then
      ...
      print*,'magnetogram of',nx1,'by',nx2,'pixels.'
    endif
  ...
  subroutine specialbound_usr(qt,ixI^L,ixO^L,iB,w,x)
    ...
    case(6)
      ...
      if (coordinate==Cartesian_stretched) then
        ...
        do ix3=ixOmin3,ixOmax3
          w(ix3^%3ixO^S,rho_)=w(ix3-2^%3ixO^S,rho_) + 2.d0*dxlevel(3)*tmp1(ix3-1^%3ixO^S)/Tiso&
                             *w(ix3-1^%3ixO^S,rho_)
        enddo
      end if
